...
const commerceTools = [
  ...
  {
    name: "listProductsByRegion",
    description: "List products enabled on a given region/website (US, CA, DE, FR, UK, B2B), optionally filtered by enabled/disabled status. This catalog is small enough that this is a full scan, not a sample — use it for 'show me US products' / 'which products are enabled in Germany' style questions.",
    input_schema: {
      type: "object",
      properties: {
        region: { type: "string", description: "Region/website name or substring, e.g. US, CA, DE, FR, UK, B2B." },
        status: { type: "string", description: "Optional: 'enabled' or 'disabled'." },
        limit: { type: "integer", description: "Max products to return in the list (1-100). Defaults to 20 — matchingCount always reflects the true total even if capped." },
      },
      required: ["region"],
    },
  },
